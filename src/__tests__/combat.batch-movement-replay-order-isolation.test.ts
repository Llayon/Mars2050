...
describe('batch movement replay order and initiative assignment isolation', () => {
  it('separates request order from initiative assignment on the certified Heavy path', { timeout: 120000 }, () => {
    ...
    expect(result.comparisons.fixedBaselineAssignment.semanticRequestSequenceEquivalent).toBe(false)
    expect(result.comparisons.fixedCandidateAssignment.semanticRequestSequenceEquivalent).toBe(false)
    expect(result.comparisons.fixedBaselineAssignment.initiativeAssignmentEquivalent).toBe(true)
    expect(result.comparisons.fixedCandidateAssignment.initiativeAssignmentEquivalent).toBe(true)
    expect(result.comparisons.fixedBaselineOrder.semanticRequestSequenceEquivalent).toBe(true)
    expect(result.comparisons.fixedCandidateOrder.semanticRequestSequenceEquivalent).toBe(true)
    expect(result.comparisons.fixedBaselineOrder.initiativeAssignmentEquivalent).toBe(false)
    expect(result.comparisons.fixedCandidateOrder.initiativeAssignmentEquivalent).toBe(false)
    ...
    for (const pair of Object.values(result.comparisons)) {
      expect(pair.planningActionMultisetEquivalent).toBe(true)
      expect(pair.planningActionSequenceEquivalent).toBe(true)
