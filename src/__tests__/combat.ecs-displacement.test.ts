...
describe('combat ECS displacement', () => {
  it('applies gravity pull after primary and radial AoE damage', () => {
    ...
    expect(world.stores.transform.require(1).x).toBe(150)
    expect(world.stores.transform.require(2).x).toBeLessThan(220)
    expect(world.stores.transform.require(3).x).toBe(210)
    expect(nativeActions.at(-1)).toMatchObject({
      unitId: 'nearby',
      type: 'move',
      fromX: 220,
      toX: world.stores.transform.require(2).x,
    })
  })

  it('applies sonic knockback after primary and cone damage', () => {
    const { world, nativeActions } = runNative([
    ...
    expect(world.stores.transform.require(1).x).toBeGreaterThan(150)
    expect(world.stores.transform.require(2).x).toBeGreaterThan(180)
    expect(world.stores.transform.require(3).x).toBe(170)
