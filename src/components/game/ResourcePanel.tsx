...
export function ResourcePanel({ resources, loading }: ResourcePanelProps) {
  if (loading) return <p>Загрузка...</p>

  return (
    <div className="bg-gray-800 p-4 rounded-lg shadow-lg">
      <h2 className="text-xl font-bold mb-3">Ресурсы колонии</h2>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {resources.map(r => (
          <div key={r.type} className="bg-gray-700 p-3 rounded-md">
            <div className="flex items-center justify-between">
              <span className="text-2xl">{RESOURCE_ICONS[r.type] || '❓'}</span>
              <span className="text-lg font-semibold">{Math.floor(r.amount)}</span>
            </div>
