...
export const MapRenderProfileSchema = z.object({
  ...
  cellWorldSize: z.number().positive(),
...
/** Pixel extension outside logical footprint bounds at reference render scale (pixelsPerWorldUnit * cellWorldSize). */
...
export const VisualAssetFrameSchema = z.object({
  ...
  if (data.footprint && data.footprint.length > 1) {
    ...
    for (const c of data.footprint) {
      const k = `${c.x},${c.y}`
      if (seen.has(k)) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: `Duplicate footprint (${k}) in ${data.id}` })
      }
      seen.add(k)
    }
  }
