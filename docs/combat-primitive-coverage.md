...
| Combat evolvement / rank scaling | Timed stat growth | `statGrowth`, `stat_growth` replay | `combat.stat-growth-charge.test.ts` | Implemented |
...
| Stealth while moving upgrade modifier | Upgrade modifier contract | `stealthWhileMoving` | `combat.upgrade-runtime-contract.test.ts` | Deferred |
| Stored upgrade on-death spawn modifier | Upgrade modifier contract | `onDeathSpawn` | `combat.upgrade-runtime-contract.test.ts` | Deferred |

Deferred means the key remains legal in `UpgradeConfig.modifiers` only because it
is explicitly listed in `DEFERRED_UPGRADE_MODIFIERS`. It must not be used in
balance config until runtime behavior and tests are added.
